...
function lightAndPointerInitialize(){
	$(".switch-meter .meter .light img").addClass("transparent");
}
//转速指针旋转
function rotatePointerRotate(rotateEndValue){
	...
	if(rotateEndValue>rotateValue){
		var timer=setInterval(function(){
			rotateValue++;
			$(".switch-meter .pointer .pointer-roate").css("transform","rotate("+rotateValue+"deg)");
...
function oilPointerRotate(oilEndValue){
	...
	var timer=setInterval(function(){
		oilValue++;
		$(".switch-meter .pointer .pointer-oil").css("transform","rotate(-"+oilValue+"deg)");
...
function openSwitchMeter(){
	...
	//隐藏小面板
	$(".switch-btn").addClass("hide-important");
	//加上大面板的拖拽事件
	dragDiv();
	dragDiv_pedal();
}
...
function closeSwitchShowBtn(obj){
	$(".switch-btn").removeClass("hide-important");
	closeDiv(obj);
}
//面板拖拽事件
function dragDiv(){
