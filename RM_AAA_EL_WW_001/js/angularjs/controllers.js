var mainModule=angular.module('mainModule',['routerModule','servicesModel','multimeterModule','obdscanModule','dashboardModule','xcjDataService'])

mainModule.controller('mainCtrl',['$scope','$compile','$rootScope','$state','serviceData','$timeout','multimeterService','obdscanService','pageService','xcjDataService',function($scope,$compile,$rootScope,$state,serviceData,$timeout,multimeterService,obdscanService,pageService,xcjDataService){
	//后台通讯初始化
	xcjDataService.init();
	
	$rootScope.recordHistory=function(id){
		xcjDataService.setHandle(id);
//		console.log(id+"==============");
    };
	
	//技能测试选择题答案加上触发事件====================================================================
	$rootScope.answerToActive=function (target){
		var id=target.id;
		//一旦提交了，则不能再选择答案了
		if(bSubmit){
			return;
...
//				$scope.chargeSrc(0);
	...
	$scope.goInclude=function(id){
		...
		if(aNo[0]=='sysPrinciple'){
			...
			//记录进度
			$rootScope.recordHistory("HD0"+(40+parseInt(aNo[1])));
		}
		//系统组成-------------------------------------------------
		if(aNo[0]=='sysComposition'){
			...
			$scope.jInfo=$scope.aAlertNaviText[aNo[1]];
			//记录进度
			$rootScope.recordHistory("HD0"+(46+parseInt(aNo[1])));
		}
		//诊断测量-------------------------------------------------
		if(aNo[0]=='diagnosisMeasurements'){
			...
			$scope.jInfo=$scope.aAlertNaviText[aNo[1]];
			//记录进度
			$rootScope.recordHistory("HD0"+(53+parseInt(aNo[1])));
		}
	}
	//信息页面返回元件检测和系统组成界面
...
//		//记录进度
//		$rootScope.recordHistory("H原理图或电路图弹出框"+id);
